...
var _ = Describe("CrlMaps domain test", func() {
	...
	Specify("Each URI should have an associated Element or Reference", func() {
		...
		eToEMap := uOfD1.GetElementWithURI(CrlElementToElementMapURI)
		Expect(eToEMap).ShouldNot(BeNil())
		eToEMapSource := uOfD1.GetReferenceWithURI(CrlElementToElementMapSourceReferenceURI)
		Expect(eToEMapSource).ShouldNot(BeNil())
		eToEMapTarget := uOfD1.GetReferenceWithURI(CrlElementToElementMapTargetReferenceURI)
