...
var _ = Describe("EdditorDomain tests", func() {
	...
	Specify("Refinement of OpenDiagrams list with URIs should serialize and deserialzie properly", func() {
		...
		firstMemberReference1FromURI := coreOpenDiagrams1.GetFirstOwnedReferenceRefinedFromURI(crldatastructures.CrlStringListReferenceToFirstMemberLiteralURI, hl1)
		...
		openDiagrams1FirstElementRef, err5 := crldatastructures.GetFirstMemberLiteral(openDiagrams1, hl1)
		Expect(err5).To(BeNil())
		Expect(openDiagrams1FirstElementRef).To(BeNil())
		openDiagrams2FirstElementRef, err7 := crldatastructures.GetFirstMemberLiteral(openDiagrams2, hl2)
