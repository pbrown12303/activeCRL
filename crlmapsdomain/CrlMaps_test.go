...
var _ = Describe("CrlMaps mapping tests", func() {
	...
	Describe("Individual Concept Mapping - any to any", func() {
		...
		BeforeEach(func() {
			...
			Expect(err).To(BeNil())
			definingSourceRefinement, err = uOfD.NewOwnedRefinement(definingSourceDomain, "DefiningSourceRefinement", nil, nil, trans)
			Expect(err).To(BeNil())
			...
			Expect(err).To(BeNil())
			definingTargetRefinement, err = uOfD.NewOwnedRefinement(definingTargetDomain, "DefiningTargetRefinement", nil, nil, trans)
			Expect(err).To(BeNil())
